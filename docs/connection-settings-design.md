...
AgentCore Identity には、ユーザーごとの接続状態だけを副作用なく一覧取得する API がありません。`GetResourceOauth2Token` または OAuth 付き Gateway ツールの呼び出しが、保存済みトークンの取得または新しい認可 URL の生成を行います。そのため、パネルを開いた時点で 3 サービスを自動確認せず、ユーザーが選んだサービスだけを確認します。
...
未連携サービスの確認は短命な認可 session の生成につながり、連携済みサービスでは provider API を呼びます。パネルを開くだけで 3 件の外部呼び出しを発生させず、ユーザーが選んだサービスだけを確認します。

### 最終確認状態をブラウザへ保存する

不採用です。

保存後に provider 側で認可が取り消されると表示が古くなります。初期版ではページ内の状態だけを保持し、再読み込み後は `未確認` に戻します。
...
ページ再読み込み後は、Token Vault の状態を推測せず `unknown` へ戻します。ユーザーが現在のページで接続確認を成功させた場合と、同じページのチャットから認可を完了した場合だけ `connected` を表示します。

接続状態を `localStorage` に保存すると、外部サービス側で認可を取り消した後も `連携済み` と表示するため採用しません。
...
- 1 provider の処理中は別 provider を開始できない
