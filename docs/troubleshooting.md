...
### 連携設定で「認可の待機時間を超えました」になる

接続確認（`connection_probe`）は認可完了を最大 5 分待ちます。認可リンクを開かなかった、別アカウントでログインした、コールバックが失敗した、といった場合に `authorization_timeout` になります。パネルで「再試行」し、認可をやり直してください。1 ユーザーあたり同時に進められる認可フローは 1 件です。

### 連携設定で「接続確認用のツールが構成されていません」になる

Runtime が期待する読み取りツール（`githubmcp___get_me` / `slackmcp___slack_search_channels` / `googlecal___listCalendars`）が Gateway にありません。Gateway Target の Name・スキーマ、デプロイ状態を確認してください。未連携とは別の構成不備です。

### 連携設定で「サービスへ接続できませんでした」になる

Token Vault にトークンがあっても provider API 側で失敗している可能性があります。provider 側でアプリ連携を取り消した場合は、再認可が必要です。CloudWatch Logs には provider ID と error code だけを出し、認可 URL やトークンは出しません。
